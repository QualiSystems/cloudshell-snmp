...
class TestSNMPConfiguratorInit(TestCase):
    ...
    def test_enable_disable_snmp_configurator_init(self, smp_params_helper_mock, snmp_mock):
        ...
        config = EnableDisableSnmpConfigurator(enable_disable_flow, resource_config, logger)
        config.get_service()

        smp_params_helper_mock.assert_called_once()
        snmp_mock.assert_called_once()

        with config.get_service() as service:
            pass
