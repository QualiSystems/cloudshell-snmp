...
class ReadSnmpService(object):
    ...
    def get_list(self, snmp_oid_list):
        ...
        for oid in snmp_oid_list:
            ...
            service = self._create_response_service()
            service.send_get_var_binds(oid=oid.get_oid(self._snmp_engine))
    ...
    def walk(self, snmp_oid_obj, stop_oid=None, get_subtree=True, retry_count=2,
        ...
        start_oid = univ.ObjectIdentifier(snmp_oid_obj.get_oid(self._snmp_engine).getOid())
        if stop_oid:
    ...
    def get_table(self, snmp_oid_obj, retry_count=2, get_bulk_flag=None,
                  get_bulk_repetitions=DEFAULT_GET_BULK_REPETITIONS):

        result_list = self.walk(snmp_oid_obj, get_subtree=True, retry_count=retry_count,
                                get_bulk_flag=get_bulk_flag, get_bulk_repetitions=get_bulk_repetitions)

        result_dict = QualiMibTable.create_from_list(result_list)
    ...
    def _start_dispatcher(self):
        try:
            ...
            self._logger.debug("Error during snmp data retrieving", exc_info=1)
            pass
