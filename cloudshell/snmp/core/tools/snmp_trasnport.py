...
class SnmpTransport(object):
    ...
    def add_udp_endpoint(self, snmp_engine, snmp_timeout=SNMP_TIMEOUT,
        ...
        if self._snmp_parameters.ip:
            try:
                agent_udp_endpoint = \
                    socket.getaddrinfo(self._snmp_parameters.ip,
                                       self._snmp_parameters.port,
                                       socket.AF_INET, socket.SOCK_DGRAM,
                                       socket.IPPROTO_UDP)[0][4][:2]
            except socket.gaierror:
                raise InitializeSNMPException("Failed to validate {} hostname".format(self._snmp_parameters.ip),
                                              self._logger)
        else:
            raise InitializeSNMPException("Failed to validate {} hostname".format(self._snmp_parameters.ip),
                                          self._logger)
        ...
        if isinstance(ip, IPv6Address):
            ...
            config.addTargetAddr(
                snmp_engine, 'tgt', udp6.domainName, agent_udp_endpoint, 'pms',
                snmp_timeout, snmp_retry_count
            )
        else:
            config.addSocketTransport(
                snmp_engine,
            ...
            config.addTargetAddr(
                snmp_engine, 'tgt', udp.domainName, agent_udp_endpoint, 'pms',
                snmp_timeout, snmp_retry_count
            )
