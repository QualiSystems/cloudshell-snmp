...
class Snmp(object):
    def __init__(self, timeout=SNMP_TIMEOUT,
    ...
    def get_snmp_service(self, snmp_parameters, logger):
        ...
        snmp_context = SnmpContext(snmp_parameters.context_engine_id, snmp_parameters.context_name)
    ...
    def _get_snmp_engine(self, pysnmp_params, logger):
        ...
        transport = SnmpTransport(snmp_parameters=pysnmp_params.snmp_parameters,
                                  logger=logger)
        transport.add_udp_endpoint(snmp_engine,
                                   self._snmp_timeout,
                                   self._snmp_retry_count)
        security = SnmpSecurity(py_snmp_params=pysnmp_params,
                                logger=logger)
